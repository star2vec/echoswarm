...
Grounded in the Valencia DANA flood of October 29, 2024 — 200+ fatalities, real Copernicus EMS flood polygons, real OSM road network.
...
The SOP file is append-only history. Evacuation rate improves measurably across runs.
...
2. Sentinel Hub Process API: VV-polarization backscatter (σ°) as 512×512 uint8 PNG
...
> Agent-based flood evacuation simulator with a closed LLM feedback loop: Claude generates a CERC-structured evacuation order, a 500-agent behavioral swarm (Compliant / Skeptical / Panic / Immobile) propagates it through a live Neo4j road-network graph while tracking message token decay per hop, and a Critic LLM diagnoses the failure mode and rewrites the communication SOP for the next run. Sentinel-1 SAR flood data via Copernicus CDSE. Grounded in the Valencia DANA 2024 event.
